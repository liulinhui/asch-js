...
module.exports = {
    ...
    createExpireAsset: function (name, desc, maximum, precision, strategy, allowWriteoff, allowWhitelist, allowBlacklist,allowExpire, secret, secondSecret) {
        var asset = {
            uiaAsset: {
                name: name,
                desc: desc,
    ...
    createExpireTransfer: function (currency, amount, recipientId,expireTime, secret, secondSecret) {
        var asset = {
            uiaTransfer: {
                currency: currency,
                amount: amount,
        ...
        bb.writeString(currency)
        bb.writeString(amount)
        bb.writeString(expireTime)
        bb.flip()
        var bytes = toLocalBuffer(bb)
        var fee = 0.1 * constants.coin
        return createTransaction(asset, bytes, fee, 15, recipientId, secret, secondSecret)
    },
}
